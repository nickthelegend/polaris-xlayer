# Stock credit — spend the stock, don't sell the stock

A Polaris payment mode: borrow stablecoin against tokenized equity to pay a
merchant, without closing the position. Lives in `apps/core` at `/stock`,
backed by `packages/contracts/contracts/polaris/`.
...
Polaris lets a shopper pay the merchant **without selling the shares**: a
...
`packages/contracts/contracts/polaris/`

| Contract | What it is responsible for |
|---|---|
| `PolarisEngine` | quote, open, repay, liquidate; custodies collateral per loan |
...
deployed there either. So on testnet Polaris deploys a stand-in for each,
...
npm install
npx hardhat test test/polaris.test.js              # 25 tests
npx hardhat run scripts/deploy-polaris.js --network xlayerTestnet
...
npx hardhat run scripts/e2e-polaris.js --network xlayerTestnet
```

`e2e-polaris.js` is the demo: it opens against the **live** venue print,
...
```bash
STOCK_TOKEN=0x... STABLE_TOKEN=0x... npx hardhat run scripts/deploy-polaris.js --network xlayer
```
