 ...
 * call that depends on the one before it has to wait for the node to catch
 * up. Funding the pool right after approving it reverted on a zero allowance
 * until this existed.
 */
...
async function main() {
  ...
  const engine = await (await ethers.getContractFactory("PolarisEngine")).deploy(
    stable, await oracle.getAddress(), await pool.getAddress(), deployer.address
  );
  await engine.waitForDeployment();
  console.log(`engine               ${await engine.getAddress()}\n`);

  await settled(await pool.setEngine(await engine.getAddress()));
  ...
  if (SEQUENCER_FEED) {
    await settled(await engine.setSequencerUptimeFeed(SEQUENCER_FEED, 3600));
  ...
  await settled(await engine.setAcceptedStock(stock, true));
  ...
  if (!process.env.STABLE_TOKEN) {
    await settled(await stableC.mint(deployer.address, warehouse));
  }
  const have = await stableC.balanceOf(deployer.address);
  const fund = have < warehouse ? have : warehouse;
  if (fund > 0n) {
    await settled(await stableC.approve(await pool.getAddress(), ethers.MaxUint256));
    await settled(await pool.fund(fund));
  ...
  try {
    ...
    const res = await fetch(
      ...
      { headers: { "User-Agent": "Mozilla/5.0 polaris-deploy" } }
    );
  ...
  await settled(await oracle.postPrice(stock, price, printedAt, marketOpen, source));
  ...
  const file = path.join(dir, `polaris-${network.name}.json`);
